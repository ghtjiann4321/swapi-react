...
export default class Home extends Component {
  componentDidMount() {
    this.props.fetchData(`/people/?page=1`, 1, 'people')
  }

  handleChangeInput = val => {
    this.props.setSearch(val)
    this.props.fetchData(`/people/?search=${val}&page=1`, 1, 'people')
  }

  handleChangePage = page => {
    const newPage = page + 1
    const val = this.props.searchString
    this.props.fetchData(`/people/?search=${val}&page=${newPage}`, newPage, 'people')
  }

  render() {
    const { people, totalPeopleCount, page, isLoading } = this.props
        ...
        <Grid container justify="center">
          <Grid item xs={11}>
            <PaperWrapper elevation={1}>
              ...
              <TableView data={people} dataCount={totalPeopleCount} page={page} onChangePage={this.handleChangePage} onClickButton={this.handleChangePage} isLoading={isLoading}/>
